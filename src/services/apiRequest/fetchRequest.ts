...
async function fetchRequest<T = unknown>(
  endpoint: string,
  { method = "GET", body = null, token = null, url = import.meta.env.VITE_API_URL_AUTH, headers = {} }: FetchRequestOptions = {},
): Promise<T> {
  ...
  const response = await triggerFetch(endpoint, method, token, sanitizedBody, baseUrl, headers);
